...
for i in range(20):
    ...
    while ep < N_EPISODES:
        ...
        if len(replay_buffer) > BATCH_SIZE:
            ...
            with torch.no_grad():
                ...
                q_action = torch.max(q_net.forward(batch_next_state), dim=1)[1]

                next_Q=target_net.forward(batch_next_state)[torch.arange(len(q_action)),q_action].view(BATCH_SIZE,1)
    ...
    results.to_csv("Double_DQN_results.csv")
